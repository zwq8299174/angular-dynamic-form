...
define(['angular', 'moment', 'jquery', 'Ps', 'daterange'], function (angular, moment, $) {
	...
	appDirectives.directive('fltInput', function () {
		return {
			template: function template($element, $attrs) {
				var tpl = '<div class="filter-bar form-line flt-input clearfix">\n\t\t\t\t\t\t<div class="form-left">\n\t\t\t\t\t\t\t<div class="form-tag">{{data.itemName}}</div>\n\t\t\t\t\t\t</div>\n\t\t\t\t\t\t<div class="form-right">\n\t\t\t\t\t\t\t<div class="form-item col-md-2 col-sm-12 col-xs-12">\n\t\t\t\t\t\t\t\t<ng-input class="md" placeholder="\u8BF7\u8F93\u5165..." model="inputVal" focus="focus" name="{{data.itemkey}}" change="inputChange()"></ng-input>\n\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t<div>\n\t\t\t\t\t</div>';
				return tpl;
			},
			replace: true,
			scope: {
